...
class Downsample2d(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv = nn.Conv2d(in_channels,
                              ...
                              padding=(0, 0))
    def forward(self, x):
        x = F.pad(x, pad=(0, 1, 0, 0), mode='circular')
        x = self.conv(x)
        return x
...
class Block2d(nn.Module):
    def __init__(self, dim, dim_out, groups = 8):
        super().__init__()
        self.proj = nn.Conv2d(dim, dim_out, (3, 3), padding = (1, 1))
...
class SpatialLinearAttention1d(nn.Module):
    ...
    def __init__(
        ...
        self.to_qkv = nn.Conv1d(channel_dim, hidden_dim * 3, 1, bias = False)
        self.to_out = nn.Conv1d(hidden_dim, channel_dim, 1)

    def forward(
        self, 
        x: torch.Tensor
    ):
        # x input size: (b, c, f, s)
        b, c, f, s = x.shape
        x = rearrange(x, 'b c f s -> (b f) c s')

        qkv = self.to_qkv(x).chunk(3, dim = 1)
        q, k, v = map(lambda z: rearrange(z, 'b (h c) s -> b h c s', h = self.heads,), qkv)
        ...
        out = rearrange(out, 'b h c s -> b (h c) s', h = self.heads, s=s)
        out = self.to_out(out)
        return rearrange(out, '(b f) c s -> b c f s', b = b)
...
class Unet2D(nn.Module):
    def __init__(
        ...
        temporal_attn = lambda dimension: EinopsToAndFrom('b c f s', 'b s f c', Attention(dimension, heads = attn_heads, channels_per_head = attn_channels_per_head, rotary_emb = rotary_emb))

        self.time_rel_pos_bias = RelativePositionBias(heads = attn_heads, max_distance = 128) # realistically will not be able to generate that many frames of video... yet
        ...
        self.init_conv = nn.Conv2d(channels, init_conv_channels, kernel_size=(init_conv_kernel_size, init_conv_kernel_size), padding = (init_padding, init_padding))

        self.init_temporal_attn = Residual(PreNorm2d(init_conv_channels, temporal_attn(init_conv_channels)))
        ...
        self.cond_conv = nn.Sequential(
            nn.Conv1d(cond_channels, init_conv_channels, kernel_size=1, stride=1, padding=0),
            nn.GELU(),
            nn.Conv1d(init_conv_channels, init_conv_channels, kernel_size=init_conv_kernel_size, stride=1, padding=init_padding)
        )
        ...
        block_klass = partial(ResnetBlock2d, groups = resnet_groups)
        ...
        for ind, (channels_in, channels_out) in enumerate(in_out_channels):
            ...
            self.downs.append(nn.ModuleList([
                ...
                Residual(PreNorm2d(channels_out, SpatialLinearAttention1d(channels_out, heads = attn_heads))) if use_sparse_linear_attn else nn.Identity(),
                Residual(PreNorm2d(channels_out, temporal_attn(channels_out))),
                Downsample2d(channels_out) if not is_last else nn.Identity()
            ]))

        mid_channels = channels_list[-1]
        self.mid_block1 = block_klass_cond(mid_channels, mid_channels)

        spatial_attn = EinopsToAndFrom('b c f s', 'b f s c', Attention(mid_channels, heads = attn_heads))

        self.mid_spatial_attn = Residual(PreNorm2d(mid_channels, spatial_attn))
        self.mid_temporal_attn = Residual(PreNorm2d(mid_channels, temporal_attn(mid_channels)))
        ...
        for ind, (channels_in, channels_out) in enumerate(reversed(in_out_channels)):
            ...
            self.ups.append(nn.ModuleList([
                ...
                Residual(PreNorm2d(channels_in, SpatialLinearAttention1d(channels_in, heads = attn_heads))) if use_sparse_linear_attn else nn.Identity(),
                Residual(PreNorm2d(channels_in, temporal_attn(channels_in))),
                Upsample2d(channels_in) if not is_last else nn.Identity()
            ]))
...
class Unet3D(nn.Module):
    def __init__(
        ...
        self.init_temporal_attn = Residual(PreNorm3d(init_conv_channels, temporal_attn(init_conv_channels)))
        ...
        for ind, (channels_in, channels_out) in enumerate(in_out_channels):
            ...
            self.downs.append(nn.ModuleList([
                block_klass_cond(channels_in, channels_out),
                block_klass_cond(channels_out, channels_out),
                Residual(PreNorm3d(channels_out, SpatialLinearAttention2d(channels_out, heads = attn_heads))) if use_sparse_linear_attn else nn.Identity(),
                Residual(PreNorm3d(channels_out, temporal_attn(channels_out))),
                Downsample3d(channels_out) if not is_last else nn.Identity()
            ]))
        ...
        self.mid_spatial_attn = Residual(PreNorm3d(mid_channels, spatial_attn))
        self.mid_temporal_attn = Residual(PreNorm3d(mid_channels, temporal_attn(mid_channels)))
        ...
        for ind, (channels_in, channels_out) in enumerate(reversed(in_out_channels)):
            ...
            self.ups.append(nn.ModuleList([
                block_klass_cond(channels_out * 2, channels_in),
                block_klass_cond(channels_in, channels_in),
                Residual(PreNorm3d(channels_in, SpatialLinearAttention2d(channels_in, heads = attn_heads))) if use_sparse_linear_attn else nn.Identity(),
                Residual(PreNorm3d(channels_in, temporal_attn(channels_in))),
                Upsample3d(channels_in) if not is_last else nn.Identity()
            ]))
