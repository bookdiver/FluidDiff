...
class Block(nn.Module):
    def __init__(self, dim, dim_out, groups = 8):
        super().__init__()
        self.proj = nn.Conv2d(dim, dim_out, kernel_size=(1, 3), stride=(1, 1), padding=(0, 1))
...
class SpatialLinearAttention(nn.Module):
    ...
    def forward(
        ...
        out = rearrange(out, 'b h c s -> b (h c) s', h = self.heads)
        out = self.to_out(out)
        return rearrange(out, '(b f) c s -> b c f s', f = f)
