...
class NavierStokesPINNDataset(Dataset):
    def __init__(self, 
                 fileroot: str,
                 filename: str,):
        
        with h5py.File(fileroot+filename, 'r') as f:
            dd = torch.from_numpy(f['density'][:]).float()
            uvuv = torch.from_numpy(f['velocity'][:]).float()
            pp = torch.from_numpy(f['pressure'][:]).float()
            t = torch.from_numpy(f['t'][:]).float()

            x = torch.linspace(0.0, 64.0, 64)
            y = torch.linspace(0.0, 64.0, 64)
            XX, YY = torch.meshgrid(x, y, indexing='xy')
            XX = XX[None, :, :].repeat(len(t), 1, 1)
            YY = YY[None, :, :].repeat(len(t), 1, 1)
            x, y = XX.flatten(), YY.flatten()

            d = dd.flatten()
            u, v = torch.chunk(uvuv, 2, dim=-1)
            u, v = u.flatten(), v.flatten()
            p = pp.flatten()

            t = t[:, None].repeat(1, 64*64).flatten()

            # concatenate all to a tuple (x, y, t, u, v, p, d, u0, v0, p0, d0)
            self.data = torch.stack([x, y, t, u, v, p, d], dim=-1)
...
if __name__ == '__main__':
    dataset = NavierStokesPINNDataset('../../../data/', 'ns_floating_sim_2d_no.800.h5')
